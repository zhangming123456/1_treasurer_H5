...
class RequestParams {
  ...
  post (type) {
    let that = this
    return axios.post(config[type], util.queryString.stringify(that.params))
  }
}
...
let ApiService = {
  ...
  mutations: {
    async ['ApiService.toLogin'] (state, payload) {
      ...
      state.qrcodeImg = `resId=${state.loginInfo.resId}&shiroUserId=${state.loginInfo.shiroUserId}`
    },
    ...
    async ['ApiService.ChinaAddressData'] (state, payload) {
      // let arr = [], data = payload.data
      // for (let v of data) {
      //   arr.push({name: v.name, value: v.id, parent: v.pid})
      // }
      // state.ChinaAddressData = state.ChinaAddressData.concat(arr)
    }
  },
  actions: {
    /**
        ...
        p.then(
          (rsp) => {
            ...
            if (2000 == rsp.data.code && util.isEmptyValue(data) && data[0] && util.isEmptyValue(data[0])) {
              ...
              dispatch('ApiService.getRegionByPid', {pid: data[0].id})
